...
class WishlistController extends GetxController implements HTTPErrorHandler {
  ...
  addToWishlist(Product item, Function callback) {
    Map<String, dynamic> items = {"items": []};
    var inList =
        wishlistItems.firstWhere((p) => p.sku == item.sku, orElse: () => null);
    if (inList != null) {
      wishlistItems.removeWhere((p) => p.sku == item.sku);
    } else {
      items["items"].add(item.sku);
    }

    wishlistItems.forEach((item) => items["items"].add(item.sku));
  ...
  removeFromWishlist(int index) {
    ...
    var removedCopy = List<Product>.from(wishlistItems);
    var removedItem = removedCopy.removeAt(index);
    if (removedItem != null) {
      removedCopy.forEach((product) => data["items"].add(product.sku));
    }
